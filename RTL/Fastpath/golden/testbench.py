...
async def fastpath_all(dut):
    random.seed(7)
    with open("golden_vectors_v3.jsonl") as f:
...
if __name__ == "__main__":
    ...
    def find_cocotb_makefiles():
        candidates = []
        try:
            import cocotb as _cocotb
            cocotb_dir = Path(_cocotb.__file__).resolve().parent
            candidates.extend([
                cocotb_dir / "share" / "makefiles",
                cocotb_dir.parent / "cocotb_tools" / "makefiles",
            ])
        except Exception:
            pass

        for candidate in candidates:
            if (candidate / "Makefile.sim").exists():
                return candidate
        ...
        try:
            ...
            raise SystemExit(
                "Could not locate Cocotb's Makefile runner. "
                "Make sure cocotb-config is on PATH, or use a cocotb "
                "version that provides cocotb_tools.runner."
            ) from exc
    ...
    try:
        ...
        if sim != "icarus":
            raise SystemExit(
                "This fallback runner only supports SIM=icarus. "
                "Install cocotb_tools for other simulators.")
...
SIM_BUILD := sim_build
COMPILE_ARGS += -g2012 -I.
export PYTHONPATH := .:$(PYTHONPATH)
include Makefile.sim
"""
        runner_makefile = proj_path / "_cocotb_legacy_runner.mk"
        runner_makefile.write_text(makefile)
        try:
            subprocess.run(
                [
                    "make",
                    "-I", str(cocotb_makefiles),
                    f'PYTHON_BIN="{sys.executable}"',
                    "-f", str(runner_makefile),
                ],
                cwd=str(proj_path),
                check=True,
            )
